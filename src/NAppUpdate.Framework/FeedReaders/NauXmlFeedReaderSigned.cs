...
namespace NAppUpdate.Framework.FeedReaders
{
	public class NauXmlFeedReaderSigned : NauXmlFeedReader
	{
		...
		private static bool VerifyHash(Byte[] hash, string publicKey, string sig)
		{
			...
			{
				throw new FeedReaderException(
					"Could not read public key. It needs to have an xml format, as required by RSACryptoServiceProvider");
			}

			if (!provider.PublicOnly)
			{
				throw new FeedReaderException(
					"The given public key contains both private and public parts. Please dont publish your private keys publicly.");
			}
